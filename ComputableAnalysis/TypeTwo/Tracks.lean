...
* The explicit arithmetic these need: `addCode`, `doubleCode`, `caseszCode`, and
  `div2mod2Code`, each with a total evaluation law.
-/
...
Closed first-order terms with total evaluation laws. These live here rather than behind
`Nat.Partrec.Code.exists_code` precisely so that the track kit depends on no axioms. -/
