...
theorem eval_limCylIndexCode (p : Baire) (m : ℕ) :
  ...
  · rw [h, eval_caseszCode_zero, if_pos rfl, eval_left]
  · rw [h, eval_caseszCode_one _ (eval_left p m), if_neg one_ne_zero]
...
theorem Lim.isCylinder : IsCylinder Lim := by
  ...
  have hzdom : Lim.Dom (cylTable w) := by
    ...
    · refine ⟨0, fun t _ => ?_⟩
      ...
      rw [if_pos h, Baire.evenPart_apply]
  ...
  have hlim : Lim.accepts x.2 y'.oddPart := by
    ...
    rw [h2] at h
    rw [h, Baire.oddPart_apply]
  exact ⟨y', by simp, (y'.evenPart, y'.oddPart),
